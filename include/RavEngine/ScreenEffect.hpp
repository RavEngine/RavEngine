...
template<typename T>
struct ScreenEffectPassInstance {
    ScreenEffectPassInstance(Ref<T> effect, const PostProcessInstanceConfig& config ) : effect(effect), inputConfiguration(config.inputconfiguration), outputConfiguration(config.outputConfiguration), clearOutputBeforeRendering(config.clearOutputBeforeRendering) {};
    ~ScreenEffectPassInstance() = default;
