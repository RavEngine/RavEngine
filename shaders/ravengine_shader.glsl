...
vec3 ComputeWorldSpacePos(vec2 screenUV, float depth, mat4 invViewProj){
	vec4 positionCS = ComputeClipSpacePosition(screenUV, depth);
	vec4 hpositionWS = invViewProj * positionCS;
	return (hpositionWS / hpositionWS.w).xyz;
}

vec3 ComputeViewSpacePos(vec2 screenUV, float depth, mat4 invProj){
	vec4 positionCS = ComputeClipSpacePosition(screenUV, depth);
