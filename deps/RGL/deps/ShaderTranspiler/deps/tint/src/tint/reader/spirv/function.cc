...
class StructuredTraverser {
  ...
  private:
    ...
    void VisitBackward(uint32_t id) {
        ...
        if (static_cast<spv::Op>(opcode) == spv::Op::OpBranchConditional) {
        ...
        } else if (static_cast<spv::Op>(opcode) == spv::Op::OpBranch) {
            VisitBackward(terminator->GetSingleWordInOperand(0));
        } else if (static_cast<spv::Op>(opcode) == spv::Op::OpSwitch) {
...
TypedExpression FunctionEmitter::MakeAccessChain(const spvtools::opt::Instruction& inst) {
    ...
    for (uint32_t index = first_index; index < num_in_operands; ++index) {
        ...
        const auto pointer_type_id = type_mgr_->FindPointerToType(pointee_type_id, SpvStorageClass(address_space));
